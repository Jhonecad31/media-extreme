    ...
    <div className="bg-[#f8fafc] min-h-screen pt-8 pb-16 text-[#1c2a4b] font-sans selection:bg-[#e53935]/30">
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        ...
        <section className="mb-10">
          {dict?.experiences && (
            <div className="mb-6">
              <h2 className="text-3xl font-black text-[#0054a6] mb-8 text-center md:text-left">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {dict.experiences.items.map((exp: any, idx: number) => (
                  <div key={idx} className="group bg-white rounded-2xl border border-slate-100 shadow-sm p-8 flex flex-col items-center text-center hover:shadow-lg hover:-translate-y-1 transition-all duration-300 relative overflow-hidden">
                    ...
                    {/* Icon Container */}
                    <div className="w-16 h-16 rounded-2xl bg-[#0054a6]/10 flex items-center justify-center mb-6 text-[#0054a6] group-hover:scale-110 group-hover:bg-[#e53935] group-hover:text-white transition-all duration-300 shadow-sm">
                      <i className={`bx ${exp.icon} text-3xl`}></i>
                    </div>

                    {/* Title */}
                    <h3 className="font-bold text-lg md:text-xl text-[#1c2a4b] mb-3 leading-snug group-hover:text-[#0054a6] transition-colors">
                      {exp.title}
                    </h3>
                    ...
                    {/* Badge */}
                    {exp.badge && (
                      <span className="text-xs font-bold text-[#e53935] bg-[#e53935]/8 border border-[#e53935]/10 px-3.5 py-1.5 rounded-full mt-auto inline-block shadow-sm">
                        {exp.badge}
                      </span>
                    )}
                  </div>
