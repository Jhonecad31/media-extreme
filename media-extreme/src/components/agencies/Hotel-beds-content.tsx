              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {dict.experiences.items.map((exp: any, idx: number) => (
                  <div key={idx} className="group bg-white rounded-2xl border border-slate-100 shadow-sm p-8 flex flex-col items-center text-center hover:shadow-lg hover:-translate-y-1 transition-all duration-300 relative overflow-hidden">
                    ...
                    {/* Icon Container */}
                    <div className="w-16 h-16 rounded-2xl bg-[#0c192c]/10 flex items-center justify-center mb-6 text-[#0c192c] group-hover:scale-110 group-hover:bg-[#e2001a] group-hover:text-white transition-all duration-300 shadow-sm">
                      <i className={`bx ${exp.icon} text-3xl`}></i>
                    </div>

                    {/* Title */}
                    <h3 className="font-bold text-lg md:text-xl text-[#1c2a4b] mb-3 leading-snug group-hover:text-[#0c192c] transition-colors">
                      {exp.title}
                    </h3>
